...
const universities = graduates.map((g) => g.university).sort();
...
const contactInfo = graduates.map((g) => [ g.first_name, g.email ].join(" "));
console.log(contactInfo);


/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called uni that contains them all. Log the result. */
const uni = universities.filter((u) => u.indexOf("Uni") > -1);
console.log(uni);
...
*/
const animalNames = [];
zooAnimals.forEach((za) => animalNames.push(`Name: ${za.animal_name}, Scientific: ${za.scientific_name}`));
...
const lowerCase = zooAnimals.map((za) => za.animal_name.toLowerCase());
...
*/
const largerPopulation = zooAnimals.filter((za) => za.population < 5);
...
*/
const populationTotal = zooAnimals.reduce((accu, cur) => accu + cur.population, 0);
